  ...
  <div>
    <el-form ref="form" label-width="80px">
      ...
      <el-form-item label="SPU图片">
        <!-- 上传图片:action图片上传的地址 list-type:文件列表的类型 :on-preview:图片预览的时候会触发 :on-remove:图片删除的时候会触发 
        file-list:
        -->
...
export default {
  ...
  data() {
    return {
      dialogImageUrl: '',
      dialogVisible: false,
      // 存储spu信息属性,spu属性始化的时候是一个空的对象,在修改spu的时候,会向服务器发请求,返回spu信息(对象),在修改的时候可以利用服务器返回的这个对象收集最新的数据提交给服务器
      // 添加spu,如果是添加spu的时候并没有向服务器发请求,数据收集到哪里呀[spu],收集数据数据的时候有哪些字段呀 看文档
      spu: {
        // 三级分类的id
        category3Id: 0,
        // spu的名称
