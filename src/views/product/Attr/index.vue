  ...
  <div>
    ...
    <el-card>
      ...
      <div v-show="!isShowTable">
        ...
        <el-button type="primary" @click="addOrUpdateAttr" :disabled="attrInfo.attrValueList.length < 1"
          >保存</el-button
        >
        <el-button @click="isShowTable = true">取消</el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    addAttrValue() {
      // 向属性值的数组里面添加元素
      // attrId: 是相应的属性的id,目前而言我们是添加属性的操作,还没有相应的属性的id,目前而言带给服务器的id为undefined
      // valueName: 相应的属性值的名称
      this.attrInfo.attrValueList.push({
        // 对于修改某一个属性的时候,可以在已有的属性值基础之上新增新的属性值(新增属性值的时候,需要把已有的属性的id带上)
        attrId: this.attrInfo.id,
        valueName: '',
        // flag属性:给每一个属性添加一个标记flag,用户切换查看模式和编辑模式,好处: 每一个属性值可以控制自己的模式切换
        flag: true
      })
      this.$nextTick(() => {
    ...
    updateAttr(row) {
      ...
      this.isShowTable = false
      // 将选中的属性赋值给attrInfo
      // 由于数据结构当中存在对象里面套参数,数组里面有套对象,因此需要使用深拷贝来解决这类问题
      // 深拷贝,浅拷贝在面试的时候出现的频率很高,切记达到手写深拷贝和浅拷贝
      this.attrInfo = cloneDeep(row)
      // 在修改某一个属性的时候,将相应的属性值元素添加上flag这个标记
      this.attrInfo.attrValueList.forEach((item) => {
        // 因为Vue无法探测普通的新增 property,这样书写的属性并非是响应式属性(数据变化视图跟着变化)
    ...
    toEdit(row, index) {
      row.flag = true
      // 获取input节点,实现自动聚焦
      // 需要注意:点击span的时候,切换为input变为编辑模式,但是需要注意的是,对于浏览器而言,页面的重绘和重拍是耗时间的
      // 点击span的时候.重绘重拍一个input它是需要好费时间,因此我们不可以一点击span立马获取到input
    ...
    async addOrUpdateAttr() {
      // 整理参数:1,如果用户添加很多属性值为空的不应该提交给服务器
